...
def derive_professional_planning_position(application_type: str) -> str:
    route = (application_type or "").upper()
    if "PD" in route or "LDC" in route:
        return "The proposal appears broadly capable of proceeding under permitted development, subject to confirmation of the relevant GPDO criteria and final drawing annotations."
    if "PRIOR" in route:
        return "The proposal appears capable of being considered through the prior approval route, subject to neighbour consultation and confirmation of the relevant dimensional limits."
    return "Planning permission is likely to be required and the proposal should be assessed against the relevant local plan, design and amenity policies."


def build_policy_context(authority_value: str, application_type: str, project_types_text: str) -> str:
    authority = authority_value or "the Local Planning Authority"
    project = (project_types_text or "").lower()
    route = (application_type or "").upper()
    refs = ["GPDO Schedule 2, Part 1 householder permitted development provisions where relevant"]
    if "PD" in route or "LDC" in route or any(t in project for t in ["loft", "dormer", "roof"]):
        refs.append("Class B / Class C roof enlargement and roof alteration criteria where relevant")
    if any(t in project for t in ["rear extension", "side extension", "infill", "first floor"]):
        refs.append("Class A enlargement, improvement or alteration criteria where relevant")
    if "porch" in project:
        refs.append("Class D porch criteria where relevant")
    refs.append(f"{authority} local plan / residential extensions SPD design and amenity guidance where applicable")
    refs.append("National Design Guide principles on context, character and amenity")
    return "\n".join(f"- {r}" for r in refs)


def ensure_planning_report_sections(report_text: str, application_type: str, project_summary: str, authority: str) -> str:
    """Make sure the clean report sections exist even if the model omits one."""
    if not report_text:
        return report_text
    text = report_text
    ...
    def insert_before(target_heading: str, block: str) -> None:
        nonlocal text
        heading = block.split("\n", 1)[0].upper()
        if has_heading(heading):
            return
        m = re.search(rf"\n{re.escape(target_heading)}\n", text)
        if m:
            text = text[:m.start()+1] + block.strip() + "\n" + text[m.start()+1:]
        else:
            text = text.rstrip() + "\n\n" + block.strip() + "\n"

    route_upper = (application_type or "").upper()
    if "PD" in route_upper or "LDC" in route_upper:
        snapshot_rows = [
            "- Relevant GPDO route: Appears capable subject to confirmation",
            "- Dimensional limits: Requires final confirmation from drawings",
            "- Materials and detailing: Matching / sympathetic materials should be annotated",
            "- Neighbour amenity: No unacceptable impact identified from the information reviewed, subject to final checks",
            "- Recommended application route: Lawful Development Certificate (Proposed)",
        ]
    elif "PRIOR" in route_upper:
        snapshot_rows = [
            "- Relevant route: Larger Home Extension Prior Approval",
            "- Rear projection and height: Requires confirmation from the submitted drawings",
            "- Neighbour consultation: Required as part of the prior approval process",
            "- Amenity considerations: Outlook, light and neighbouring impact should be clearly reviewed",
            "- Recommended application route: Prior Approval",
        ]
    else:
        snapshot_rows = [
            "- Relevant route: Full householder planning permission",
            "- Design and scale: To be assessed against local residential extension guidance",
            "- Streetscene impact: Requires assessment against local character and design policies",
            "- Neighbour amenity: Outlook, privacy and light impacts should be reviewed",
            "- Recommended application route: Householder Planning Application",
        ]

    insert_before("PLANNING ASSESSMENT", "COMPLIANCE SNAPSHOT\n" + "\n".join(snapshot_rows) + "\n")

    conclusion = (
        "PROFESSIONAL CONCLUSION\n"
        f"Based on the submitted information, {project_summary.rstrip('.').lower()} has been reviewed against the relevant planning route and policy considerations. "
        f"{derive_professional_planning_position(application_type)} "
        "The drawings should be checked and annotated to address the outstanding matters before formal submission.\n"
    )
    insert_before("SUBMISSION READINESS", conclusion)
    return text
...
TOP SUMMARY
- Keep this section very clean and client-ready.
- Do not include risk ratings, confidence scores, planning approval probabilities, AI wording, or backend rule language.
- Include only these key-value lines, followed by no more than 3 short planning consideration bullets:
  - Project Summary: {project_summary_value}
  - Likely Planning Route: {application_type_value}
  - Overall Planning Position: {derive_professional_planning_position(application_type_value)}
...
- Refer to relevant policy sources only, using this policy reference context:
{build_policy_context(authority_value, application_type_value, project_types_text)}
...
COMPLIANCE SNAPSHOT
- Provide 4 to 6 concise status bullets only.
- Each line must follow this format: Requirement: Status / comment.
- Use statuses such as Appears compliant, Requires confirmation, Not clearly shown, or Not applicable.
- For loft/dormer schemes include roof volume, ridge height, eaves setback, front roof slope, materials, and side-facing glazing where relevant.
- For rear extensions include projection, height, eaves/boundary relationship, materials, and neighbour amenity where relevant.
- Do not include long policy explanation in this section.
...
- Avoid generic warnings and do not repeat missing-information points unless they affect the route.
...
PROFESSIONAL CONCLUSION
- Add one short professional conclusion paragraph.
- Confirm the likely route and next step.
- Do not repeat the full report.
- Do not use AI wording.
