...
def render_kpi_cards(sections: Dict[str, str], report_id: str, module_name: str):
    ...
    label_1 = "Planning Position" if module_name == "Planning Review" else "Risk Rating"
    ...
    if module_name == "Planning Review":
        ...
        for line in sections.get("TOP SUMMARY", "").splitlines():
            ...
            if stripped and ":" not in stripped:
                ...
                break
        return {
            "risk": top_summary_rows.get("OVERALL PLANNING POSITION", "Professional review"),
            "route": top_summary_rows.get("LIKELY PLANNING ROUTE", top_summary_rows.get("LIKELY ROUTE", top_summary_rows.get("APPLICATION TYPE", "Unknown"))),
...
with st.sidebar:
    ...
    report_logo = st.file_uploader("Report Logo (optional)", type=["png", "jpg", "jpeg"], key="report_logo_upload")
    if report_logo is not None:
        st.session_state["brand_logo_bytes"] = report_logo.getvalue()
        st.caption("Logo will be used on the exported PDF reports.")
    elif st.session_state.get("brand_logo_bytes"):
        st.caption("Logo saved for this session.")
...
with upload_tab:
    ...
    if uploaded_file and run_analysis:
        ...
        try:
            ...
            clean_practice_name = clean_input_value(practice_name, SY_BRAND["practice_name"])
        ...
        with insight_col:
            ...
            if review_module == "Planning Review":
                ...
                if st.button("Generate Planning Statement", key="generate_planning_statement_btn", use_container_width=True):
                    ...
                    st.session_state["planning_statement_file"] = build_simple_word_doc("Planning Statement", statement_text)
